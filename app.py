...
def is_crime_related(headline: str, client: Groq) -> bool:
    ...
    try:
        ...
        logging.debug(f"Groq crime check for '{headline}': Result='{result}'")
        return result == "true"
    except Exception as e:
        logging.error(f"Error checking crime status with Groq for headline '{headline}': {e}")
...
def fetch_and_filter_news():
    ...
    try:
        ...
        all_scraped_data.extend(ndtv_data)
    except Exception as e:
    ...
    try:
        ...
        all_scraped_data.extend(ani_data)
    except Exception as e:
    ...
    if not groq_client:
        ...
        # Decide behavior: return all news or none? Let's return all for now.
        filtered_ndtv_data = ndtv_data
        filtered_ani_data = ani_data
    else:
        logging.info("Filtering news items for crime relevance using Groq...")
        # Filter NDTV Data
        for item in ndtv_data:
            headline = item.get("content", "")
            if headline and is_crime_related(headline, groq_client):
                logging.info(f"CRIME (NDTV): {headline}")
                filtered_ndtv_data.append(item)
            else:
                logging.debug(f"NON-CRIME (NDTV): {headline}")

        # Filter ANI Data
        for item in ani_data:
            headline = item.get("content", "") # Ensure ANI data has 'content' key
            if headline and is_crime_related(headline, groq_client):
                logging.info(f"CRIME (ANI): {headline}")
                filtered_ani_data.append(item)
            else:
                logging.debug(f"NON-CRIME (ANI): {headline}")

        logging.info(f"Filtering complete. Found {len(filtered_ndtv_data)} crime items from NDTV, {len(filtered_ani_data)} from ANI.")
